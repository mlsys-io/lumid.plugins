...
A single ``install()`` returns one ``BaseBindings``. Identity is always
registered; the jobs ACL surface and the remote-optimizer surface activate
only when their env vars are set, so identity-only deployments need no extra
config.
"""
